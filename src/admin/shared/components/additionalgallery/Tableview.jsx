...
const Tableview = (props) => {
  ...
  // Process tabledata to add Type field
  const processedTableData = tabledata.map(item => ({
    ...item,
    Type: item.Images && item.Images.length > 0 ? 'Image' : item.Videos && item.Videos.length > 0 ? 'Video' : 'None'
  }));
  ...
  const mediaTemplate = (rowData) => {
    if (rowData.Type === 'Image' && rowData.Images && rowData.Images.length > 0) {
      return (
    ...
    } else if (rowData.Type === 'Video' && rowData.Videos && rowData.Videos.length > 0) {
      return (
        <div className="flex justify-center">
          <button
            onClick={() => {
  ...
  const departmentTemplate = (rowData) => {
    return rowData.Department_details?.Department_name || 'N/A';
  };
  ...
  const columns = [
    ...
    { field: 'Department_details.Department_name', header: 'Department', filter: true, body: departmentTemplate },
    { field: 'Status', header: 'Status', filter: true, body: statusTemplate },
    { field: 'createdAt', header: 'Created On', filter: false, body: createdAtTemplate }
  ];
