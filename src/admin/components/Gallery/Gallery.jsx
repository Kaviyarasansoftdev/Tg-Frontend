...
//         // Check if we have combined files in e.target.value (from our custom handler)
//         if (e.target.value && Array.isArray(e.target.value)) {
//             // Use the combined files array
//             setFormdata({...formdata, [e.target.name]: e.target.value});
//         } else {
//             // Original behavior for new uploads only
//             setFormdata({...formdata, [e.target.name]: filesArray});
//         }
    ...
    if (e.target && e.target.files) {
        ...
        if (formdata.Src) {
            if (typeof formdata.Src === 'string') {
                existingMedia = [formdata.Src];
            } else if (Array.isArray(formdata.Src)) {
                existingMedia = formdata.Src.filter(item => typeof item === 'string'); // Only keep string URLs
            }
        }

        // Combine existing media with new files
        const combinedFiles = [...existingMedia, ...newFiles];

        // Update formdata with combined files
        setFormdata({ ...formdata, [e.target.name]: combinedFiles });

        // Update dataUrl for preview (only for the first new file)
        const file = e.target.files[0];
        if (file) {
            const reader = new FileReader();
            reader.onloadend = () => {
                setDataUrl({ src: reader.result, length: newFiles.length });
            };
            reader.readAsDataURL(file);
        }
    } else if (e.target && !e.target.files) {
