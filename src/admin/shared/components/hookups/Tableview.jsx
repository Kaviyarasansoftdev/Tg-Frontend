...
//   const { tabledata, editfrom, handledelete, cusfilter, filtervalues, onPage, page ,totalRecords, loading} = props;
...
//   const columns = [
//     { field: 'Tags', header: 'Tags Name', filter: true },
//     { field: 'Brand_Name', header: 'Brand_Name', filter: true },
...
const Tableview = (props) => {
  const { 
    ...
    totalRecords, 
    loading,
    columns = [] // Accept dynamic columns from parent
  } = props;
  ...
  useEffect(() => {
    // Load filter options for each column
    columns.filter(col => col.filter).forEach(async (col) => {
      try {
        const options = await getFilterOptions(col.field);
        if (options && options[col.field]) {
          setFilterOptions(prev => ({
            ...prev,
            [col.field]: options[col.field].map(val => ({
              label: val,
        ...
        console.error(`Error loading filter options for ${col.field}:`, error);
      }
    });
  }, [columns]);
