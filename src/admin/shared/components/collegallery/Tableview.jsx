...
const Tableview = (props) => {
  ...
  const typeTemplate = (rowData) => {
    ...
    return (
      <span className={`px-3 py-1 rounded-full text-sm font-medium ${getTypeColor(rowData.Type)}`}>
        {rowData.Type}
      </span>
    );
  };
  ...
  const mediaTemplate = (rowData) => {
    if (rowData.Type === 'Images' && rowData.Images && rowData.Images.length > 0) {
      return (
    ...
    } else if (rowData.Type === 'Videos' && rowData.Videos && rowData.Videos.length > 0) {
      return (
        <div className="flex justify-center">
          <button
            onClick={() => {
              setSelectedVideo(`${apiurl()}/${rowData.Videos[0]}`);
            ...
            title="Play Video"
          >
            <i className="text-2xl text-blue-600 group-hover:text-blue-700 group-hover:scale-110 transition-all duration-200 fi fi-rr-play"></i>
          </button>
        </div>
      );
    }
  ...
  const eventDateTemplate = (rowData) => {
    return moment(rowData.Event_Date).format('MMM DD, YYYY');
  };

  const createdAtTemplate = (rowData) => {
    return moment(rowData.createdAt).format('MMM DD, YYYY HH:mm');
  };

  const mediaCountTemplate = (rowData) => {
    if (rowData.Type === 'Images') {
      const imageCount = rowData.Images ? rowData.Images.length : 0;
      return (
        <span className="text-sm text-gray-600">
          {imageCount} {imageCount === 1 ? 'image' : 'images'}
        </span>
      );
    } else if (rowData.Type === 'Videos') {
      const videoCount = rowData.Videos ? rowData.Videos.length : 0;
      return (
        <span className="text-sm text-gray-600">
  ...
  const columns = [
    ...
    { field: 'Type', header: 'Type', filter: true, body: typeTemplate },
    { field: 'Event_Date', header: 'Event Date', filter: false, body: eventDateTemplate },
    { field: 'Status', header: 'Status', filter: true, body: statusTemplate },
    { field: 'createdAt', header: 'Created', filter: false, body: createdAtTemplate }
  ];
    ...
    <Dialog
      header={
        <div className="flex items-center gap-3">
          <i className="text-lg text-blue-600 fi fi-rr-play"></i>
          <span className="text-lg font-semibold text-gray-800">{videoTitle}</span>
        </div>
      }
        ...
        {selectedVideo ? (
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              <i className="text-6xl text-gray-400 fi fi-rr-play mb-4"></i>
    ...
    <div className="p-4 bg-white border-b">
      <div className="flex items-center justify-between gap-4">
        <div className="relative flex-1 max-w-md">
          <h2 className="text-xl font-semibold text-gray-800">College Gallery</h2>
